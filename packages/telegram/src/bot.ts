// Webhook-based bot for Vercel deployment
// Contains all features from local.ts adapted for serverless
...
bot.callbackQuery('action:status', async (ctx) => {
    ...
    await ctx.reply('📊 Fetching status...')

    try {
        const prefs = await getUserPrefs(ctx.chat!.id)
        const scans = await Promise.all(
            addresses.map(addr => performScan(addr, prefs.network, prefs.customRpc ?? undefined))
        )
...
*📊 Scanning*
/scan <address> - Scan a fee payer
/status - Status of all tracked

*📌 Tracking*
/track <address> - Start tracking
...
bot.command('scan', async (ctx: Context) => {
    ...
    try {
        const prefs = await getUserPrefs(ctx.chat!.id)
        const result = await performScan(address, prefs.network, prefs.customRpc ?? undefined)
...
bot.command('untrack', async (ctx: Context) => {
    ...
    if (removed) {
        await ctx.reply(`✅ Stopped tracking \`${shortAddr}\``, { parse_mode: 'Markdown' })
    } else {
...
// Export bot instance for setting webhook
export { bot }
